...
import { nameFromPath, objFirstKey } from 'src/utils';
	...
	return (
		<div id={'list-item-' + (idx + 1).toString()}>
			<div onClick={() => setChosenFile(file)} className={'relative flex rounded-lg h-20 overflow-hidden cursor-pointer ' + (completedFiles.includes(file) ? 'bg-[hsl(100,25%,15%)]' : 'bg-neutral-800')}>
				<img key={objFirstKey(poznavacka!) + idx} src={file.replace(' ', '%20').replace('+', '%2b')} alt={`${objFirstKey(poznavacka!)} - obrázek ${idx + 1}`} className='object-cover aspect-square' />

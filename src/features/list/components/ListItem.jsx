...
function ListItem({ idx, file }) {
	...
	return (
		<div id={'list-item-' + (parseInt(idx) + 1)}>
			<div onClick={(e) => setChosenFile(file)} className={'relative flex rounded-lg h-20 overflow-hidden cursor-pointer ' + (completedFiles.includes(idx + 1) ? 'bg-[hsl(100,25%,15%)]' : 'bg-neutral-800')}>
				...
				<div className='relative flex flex-grow items-center'>
					...
					<div className={'top-0 right-2 z-10 absolute font-black text-xl ' + (completedFiles.includes(idx + 1) ? 'text-lime-600' : 'text-neutral-600')}>{idx + 1}</div>
				</div>
			</div>
		</div>
	);
